...
rearrangePoints[list:{_?NumericQ,_?NumericQ,__?NumericQ}]:=Module[{nMax=Length@list,l},
	l=(N@list[[#]]{Cos[(((#-1)2/nMax)+1/2) Pi],Sin[(((#-1)2/nMax)+1/2) Pi]})&/@Range@nMax;Append[l,First@l]]
	
rearrangePoints[list:{{_?NumericQ,_?NumericQ,__?NumericQ}...}]:=rearrangePoints[#]&/@list

(*spacing is used to automate the Axis Scaling *)

spacing[value_]:=Module[{log,mod,int},log=N@Log[10,value];
	mod=Mod[log,1];
	int=Floor[log];
...
CycleValues[list_List,n_Integer]:=Module[{hold=list},While[Length[hold]<n,hold=Join[hold,hold]];
  Take[hold,n]/.None->{}]
...
dsToSequence[ds_Dataset/;Length@Dimensions@ds==2,opts:OptionsPattern[]] := Module[{keys = ds[1, Keys] // Normal,
  ...
  dmzList[values, 
  	If[chartLegends===Automatic,ChartLegends -> keyString,ChartLegends->None], 
    If[axesLabel===Automatic,AxesLabel -> keyValues,AxesLabel->None]] /. dmzList -> Sequence]
...
dsToSequence[ds_Dataset /; Length@Dimensions@ds == 1,opts:OptionsPattern[]] := 
 Module[{keys = ds[Keys] // Normal, 
  ...
  dmzList[values, If[chartLegends===Automatic,ChartLegends -> keyString,ChartLegends->None], 
    If[axesLabel===Automatic,AxesLabel -> keyValues,AxesLabel->None]] /. dmzList -> Sequence]
...
concatenate[{a_String}]:=a
concatenate[{a_String, b_String, c__}] := 
concatenate[Flatten[{StringJoin[a, ", ", b], c}]]
concatenate[{a_String, b_String}] := Style[StringJoin[a, ", ", b]]
concatenate[list : {_List ..}] := concatenate[#] & /@ list
...
RadarChart[list:{{_?NumericQ,_?NumericQ,__?NumericQ}...},opts:OptionsPattern[]]:=
	Module[
			{min,max,length,vars,nbrVars,axes,ticks,tickLabels,steps,colors,polys,maxAxis,labelCoeff,
		...
		,
			If[Length@Dimensions@list!=2,Message[RadarChart::badDim];Abort[]];
			length=Length@First@list;
  			nbrVars=Length@list;
  			vars=rearrangePoints[list];
  			filling=If[filling===None,Transparent,Opacity[0.3]];
  			labelCoeff=If[axesType=="Star",1.18,1.05];
  			If[plotStyle===Automatic,
  				colors=ColorData[1][#]&/@Range@nbrVars;
          		polys=Transpose[{{FaceForm[{#,filling}],EdgeForm[{Thick,#}]}&/@colors
      		,
           		Polygon[#]&/@vars}],plotStyle=CycleValues[plotStyle,nbrVars];
  			...
  			Which[ 
	  			...
	  			Head@chartLegends===List,
	  				If[First[Head/@chartLegends]==String,chartLegends=Style/@chartLegends];
...
RadarChart[list:{_?NumericQ,_?NumericQ,__?NumericQ},opts:OptionsPattern[]]:=RadarChart[{list},opts]
...
RadarChart[assn_Association,opts:OptionsPattern[]]:=RadarChart[Dataset[{assn}],opts];
RadarChart[assn:{_Association...},opts:OptionsPattern[]]:=RadarChart[Dataset[assn],opts];
